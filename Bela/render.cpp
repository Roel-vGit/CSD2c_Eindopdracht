...
void sendOSC(void*)
{
	while(!Bela_stopRequested()) //checks if the prorgram should end
	{
		...
		yPos = yPos * 2.0f - 1.0f;
		if (xPos < 0.01f && xPos > 0.001f)
			xPos = 0.0f;
		if (yPos < 0.03f && yPos > 0.015f)
			yPos = 0.0f;
...
void render(BelaContext *context, void *userData)
{
	...
	for(unsigned int n = 0; n < context->audioFrames; n++) {
		if(gAudioFramesPerAnalogFrame && !(n % gAudioFramesPerAnalogFrame)) {
			...
			yPos = map(analogRead(context, n/gAudioFramesPerAnalogFrame, joystickX), 0.0f, 0.82f, 0.0f, 1.0f);
			xPos = map(analogRead(context, n/gAudioFramesPerAnalogFrame, joystickY), 0.0f, 0.82f, 0.0f, 1.0f);
			yPos2 = map(analogRead(context, n/gAudioFramesPerAnalogFrame, joystick2X), 0.0f, 0.82f, 0.0f, 1.0f);
			xPos2 = map(analogRead(context, n/gAudioFramesPerAnalogFrame, joystick2Y), 0.0f, 0.82f, 0.0f, 1.0f);
		}
	}
}	
